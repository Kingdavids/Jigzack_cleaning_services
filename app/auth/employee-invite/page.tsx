...
type InviteCheck = { valid: boolean; email?: string | null };
...
export default async function EmployeeInvitePage({
    ...
    if (typeof token === "string" && token.length > 0 && token.length <= 128) {
        ...
        const { data } = await supabase.rpc("check_employee_invite", { p_token: token });
                ...
                <section className="rounded-2xl border border-white/10 bg-white/[0.03]">
                    <div className="px-5 py-6 md:px-7 md:py-8">
                        {invite.valid && typeof token === "string" ? (
                            <Signup inviteToken={token} presetEmail={invite.email ?? null} />
                        ) : (
                            <div className="text-center">
                                ...
                                <p className="mt-3 text-sm leading-6 text-white/65">
                                    It may have expired or already been used. Ask an admin to send you a new
                                    employee invite.
                                </p>
                                <Link
                                    href="/"
